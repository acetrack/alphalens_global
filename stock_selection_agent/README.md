...
## ⚠️ 필수 요구사항: 데이터 신선도 검증 (Data Freshness)

**분석 기준일과 데이터 기준일이 다를 수 있으므로 반드시 검증해야 합니다.**

### 왜 중요한가?
- 주가 데이터가 며칠 전 데이터일 수 있음 (예: 분석일 2/1, 주가 데이터 1/19)
- 오래된 주가로 밸류에이션 계산 시 부정확한 투자 판단 초래
- 투자자에게 데이터의 시점을 명확히 전달해야 함

### 데이터 유형별 허용 기준
| 데이터 유형 | 경고 기준 | 오류 기준 | 비고 |
|------------|----------|----------|------|
| 주가 | 1일 | 5일 | 가능한 최신 데이터 사용 |
| 재무 데이터 | 30일 | 90일 | 분기 발표 주기 고려 |
| 애널리스트 | 14일 | 60일 | 목표주가 유효기간 고려 |

### 리포트 필수 포함 항목

모든 분석 리포트에는 다음과 같은 **데이터 기준일 테이블**을 포함해야 합니다:
...
| 재무 데이터 | 2025-12-31 | 32일 | ✅ 최신 |
| 애널리스트 | 2026-01-15 | 17일 | ⚠️ 참고 |
```
...
분석 기준일과 **13일** 차이가 있습니다.

실제 투자 결정 시 최신 시세를 반드시 확인하세요.
```

### 설정 파일 참조
- `config/workflow.yaml`: `data_freshness` 섹션
- `agents/00_master_orchestrator/INSTRUCTIONS.md`: 데이터 신선도 검증 절차
