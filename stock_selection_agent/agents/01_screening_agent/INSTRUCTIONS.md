...
date_validation:
  required: true
  check_items:
    - current_date: "시스템 날짜 또는 사용자 확인"
    - current_year: "실적 데이터 검색 연도"
    - trailing_12m: "최근 12개월 기준점"
    - forward_estimates: "향후 실적 추정치 기준년"

  search_keywords:
    # 현재가 2026년 2월이라면:
    - "2025년 연간 실적"     # 최근 확정 실적
    - "2025년 4분기 실적"    # 최근 분기 실적
    - "2026년 실적 전망"     # Forward 추정치
    - "2026년 컨센서스"      # 애널리스트 추정
```
